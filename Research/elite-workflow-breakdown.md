...
- **Categories:** A2 (T1) · C1 (T2) · F4 (T3).
- **Volume:** 50–100 hand-reviewed envelopes per category.
- **Generation order (by context cost):** L → R → A → G.
- **Teacher:** DeepSeek API; local 7B fallback if budget is zero.
- **Metric:** pass@3 against deterministic verifiers. F4 additionally reviewed by local 7B; API review only on low confidence.
- **Decision rule:** T1/T2 pass@3 ≥ 80% → swarm-eligible. T3 ≥ 70% (post-review) → eligible; below → stays "swarm drafts, stronger reviews" in v1.

F4's G-context reuses the Postmortem schema (§8). No second schema.
